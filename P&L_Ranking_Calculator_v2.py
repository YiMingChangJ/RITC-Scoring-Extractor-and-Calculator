...
class CaseRankAnalyzer:
    ...
    def load_and_prepare(self) -> None:
        """Load results, orders, and charts (tick-level NLV)."""
    ...
    def compute_tick_stats(self) -> pd.DataFrame:
        ...
        from the 'Charts - Traders' sheet in Results.xlsx.
        """
        excel_files = sorted(glob.glob(os.path.join(self.main_path, "*", "Results.xlsx")))
        heat_counters = {"ETF": 0, "Volatility": 0, "Unknown": 0}
        tick_data = []
        ...
        for file in excel_files:
            folder_name = os.path.basename(os.path.dirname(file))
            charts = pd.read_excel(file, sheet_name="Charts - Traders") 
            if "ETF" in folder_name:
                case = "ETF"
            elif "Volatility" in folder_name:
                case = "Volatility"
            ...
            team_map = {c: c.split("-")[0] for c in nlv_cols}
        ...
        tick_df["sharpe_rank"] = tick_df.groupby(["case", "heat"])["sharpe_for_rank"].transform(lambda s: s.rank(ascending=False, method="dense"))
        ...
        avg_sharpe_case_rank['overall_sharpe_rank'] = avg_sharpe_case_rank['avg_case_rank'].rank(method='dense', ascending=True)  # higher Sharpe = better rank 
    ...
    def build_table(self) -> None:
        ...
        team_heat["heat_rank"] = team_heat.groupby(["case", "heat"])["NLV_for_rank"].transform(lambda s: s.rank(ascending=False, method="dense"))
        ...
        if self.orders_data is not None and not self.orders_data.empty:
            ...
            trans_counts['transaction_count'] = trans_counts['transaction_count'].rank(method='dense', ascending=False) 
